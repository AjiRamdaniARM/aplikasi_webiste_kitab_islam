            ...
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all w-full sm:my-8 sm:w-full sm:max-w-lg">
                ...
                @else
                    ...
                    <form wire:submit.prevent="submit" class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-4">
                                <label for="name_kitabs" class="block text-sm font-medium text-gray-700">Name Status</label>
                                <input type="text" wire:model="name_kitabs" id="name_kitabs" class="mt-3 block w-full rounded-[10px] border-2 border-gray-500 h-10 px-5 shadow-sm focus:border-orange-500 focus:ring-orange-500 sm:text-sm" required>
                                @error('name_kitabs') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                            </div>
