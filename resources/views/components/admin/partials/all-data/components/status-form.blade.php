    ...
    <div id="modal-form" class="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all w-full sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div x-data="statusForm()" x-init="init">
                    <form method="POST" @submit.prevent="submitForm" class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        @csrf
                        <div class="mt-4">
                            <label for="name_status" class="block text-sm font-medium text-gray-700">Name Status</label>
                            <input type="text" x-model="name_status" name="name_status" id="name_status" class="mt-3 block w-full rounded-[10px] border-2 border-gray-500 h-10 px-5 shadow-sm focus:border-orange-500 focus:ring-orange-50 sm:text-sm" required>
                        </div>
...
<script>
    const statusPostUrl = "{{ route('status.post') }}";
</script>
<script src="{{asset('admin/JS/sendPost.js')}}"></script>